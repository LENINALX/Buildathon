...
const respuestaSchema = {
  ...
  schema: {
    ...
    required: ["explicacion", "recomendacion", "checklist", "pregunta_sugerida"],
  },
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({
      ...
      rafaga_viento: datos.rafaga_viento,
      temp_mar: datos.temp_mar,
      periodo_ola: datos.periodo_ola,
      direccion_ola: direccionComoTexto(datos.direccion_ola),
      mejores_horarios,
      pronostico,
    ...
    return json({ error: "No fue posible confirmar las condiciones del mar. No recomendamos zarpar sin una fuente oficial." }, 503);
  }
});
...
async function generarConsejo(ubicacion: string, datos: Awaited<ReturnType<typeof obtenerDatosMar>>, nivel: string, horarios: string[]) {
  const respuesta = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: "Eres Capitán Seguro, asistente de seguridad para pescadores artesanales de Manabí. Escribe en español claro y breve. No inventes información, no contradigas el nivel de riesgo calculado y no afirmes que sea seguro zarpar. Si el riesgo es alto, recomienda no salir." },
        { role: "user", content: `Datos confirmados de ${ubicacion}: riesgo ${nivel}; oleaje ${datos.altura_ola} m; período ${datos.periodo_ola} s; viento ${datos.velocidad_viento} km/h; ráfagas ${datos.rafaga_viento} km/h; mejores horarios ${horarios.join(", ")}. Genera el consejo solo con estos datos.` },
      ],
